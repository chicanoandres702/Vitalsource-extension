...
function checkForOutline(bookId) {
    safeChrome(() => {
        chrome.storage.local.get([`outline_${bookId}`], (res) => {
            if (res[`outline_${bookId}`]) chapterTreeService.handleOutlineUpdate(res[`outline_${bookId}`]);
        });
    });
}
...
safeChrome(() => {
    chrome.runtime.onMessage.addListener((d, sender) => {
        ...
        if (d.type === 'ALIVE') {
            ...
            if (d.contextId) {
                if (ui.vesselId.textContent !== d.contextId.toUpperCase()) checkForOutline(d.contextId);
            ...
            if (!fingerprints.has(d.meta.fingerprint)) {
                ...
                if (!handledByManifest) {
                    ...
                    setTimeout(() => coordinatorService.triggerTurn(), 100);
                }
            } else {
